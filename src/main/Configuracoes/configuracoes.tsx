...
import AutoAwesomeOutlinedIcon from '@mui/icons-material/AutoAwesomeOutlined';
  ...
  const Configuracoes = () => {
    ...
    const renderAtestadoTab = (certificateLines: string) => (
      <div className="tabContent">
        <TextField
          ...
          sx={{ mb: 2 }}
        />
        <textarea defaultValue={certificateLines}></textarea>
      </div>
    );
  
    const renderReceitaTab = (prescriptionLines: string) => (
      <div className="tabContent">
        <textarea defaultValue={prescriptionLines}></textarea>
      </div>
    );
    ...
    const renderResumoTab = (summaryLines:string) => (
      <div className="tabContent">
        <textarea defaultValue={summaryLines}></textarea>
      </div>
    );

    const renderTranscricaoTab = (transcriptionLines: string) => (
      <div className="tabContent">
        <textarea defaultValue={transcriptionLines}></textarea>
      </div>
    );
